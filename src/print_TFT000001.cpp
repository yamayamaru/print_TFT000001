...
print_TFT000001::print_TFT000001(int32_t d0, int32_t wr, int32_t dc, int32_t rst, int32_t cs) :
  TFT000001(d0, wr, dc, rst, cs) {
...
#else
print_TFT000001::print_TFT000001(spi_inst_t *inst, int32_t dc, int32_t rst, int32_t cs) :
  TFT000001(inst, dc, rst, cs) {
...
void print_TFT000001::set_font_color(uint32_t fg, uint32_t bg) {
    _colormap.fg = fg;
    _colormap.bg = bg;
}

void print_TFT000001::set_font_pos(int32_t x, int32_t y) {
    _x = x;
    _y = y;
}

void print_TFT000001::set_font(font_class01 *font) {
    _font = font;
}

bool print_TFT000001::get_bit(uint8_t *glyph, int32_t ch) {
    int32_t bit, byte;
    bit = ch % 8;
    byte = ch / 8;
...
void print_TFT000001::clear_memory(uint8_t *buf, uint8_t value, uint32_t num){
    for (int32_t i = 0; i < num; i++) {
        buf[i] = value;
    }
}

int32_t print_TFT000001::chars(const char *str, int32_t x, int32_t y) {
    int32_t str_w, div, rem, nbytes, index;
    uint8_t *glyph;
    int32_t char_w, pos;

    str_w = _font->get_width(str, _PRINT_TFT000001_MAX_TEXT_SIZE01);
    div = _font->height() / 8;
    rem = _font->height() % 8;
    if (rem == 0) nbytes = div;
    else          nbytes = div + 1;
    ...
    pos = 0;
    glyph = glyph_buff01;
    int32_t str_len02 = strnlen(str, _PRINT_TFT000001_MAX_TEXT_SIZE01);
...
int32_t print_TFT000001::next_line(int32_t cury, int32_t char_h) {
    int32_t res;
...
int print_TFT000001::substr01(const char *source, char *dest, int buffsize, int start, int length) {
    char *sp, *ep, size;
    size = buffsize - 1;
    int32_t source_len01 = strnlen(source,  _PRINT_TFT000001_MAX_TEXT_SIZE01);
...
void print_TFT000001::print(const char *text) { // does character wrap, compatible with stream output
    ...
    str_len01 = strnlen(text, _PRINT_TFT000001_MAX_TEXT_SIZE01);
    if (str_len01 > _PRINT_TFT000001_STR_BUFF01_LEN01) return ;

    for (int32_t pos = 0; pos < str_len01; pos++) {
        char ch = text[pos];
        if (ch == '\n') {
            if (pos > 0) {
                substr01(text, str_buff01, _PRINT_TFT000001_STR_BUFF01_LEN01, written, pos - written);
    ...
    if (written < str_len01) {
        substr01(text, str_buff01, _PRINT_TFT000001_STR_BUFF01_LEN01, written, str_len01 - written);
...
void print_TFT000001::word_print(std::string text) { // does word wrap, leaves self._x unchanged
...
void print_TFT000001::set_font_back_ground_fill(bool flag) {
    _back_ground_fill_flag = flag;
}

#include <stdarg.h>
void print_TFT000001::printf(const char* format, ...) {

    char str_buff01[_PRINT_TFT000001_MAX_TEXT_SIZE01 + 1];

    va_list ap;
    va_start(ap, format);

    vsnprintf(str_buff01, _PRINT_TFT000001_MAX_TEXT_SIZE01, format, ap);
...
void print_TFT000001::word_printf(const char* format, ...) {
